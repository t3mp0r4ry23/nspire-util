	...
	switch(cmd) {
		...
		case 1:
			printf("%.2lf MiB of RAM used out of %.2lf MiB, leaving %.2lf MiB free.\n",
				(float) (device_info->ram.total - device_info->ram.free)/(1024*1024),
		...
		case 4:
			...
			long nspire_file_size = ftell(nspire_file);
			fseek(nspire_file, 0, SEEK_SET);

			printf("Reading data from host from file at %s...\n", argv[2]);
			char * nspire_file_buffer = malloc(nspire_file_size + 1);
			fread(nspire_file_buffer, nspire_file_size, 1, nspire_file);
			fclose(nspire_file);

			printf("Sending %.3f %s of data to calculator to save to file at %s...\n", nspire_file_size > 999999 ? ((float) nspire_file_size/(1024*1024)) : ((float) nspire_file_size/1024), nspire_file_size > 999999 ? "MiB" : "KiB", argv[3]);
			ret = nspire_file_write(handle, argv[3], nspire_file_buffer, nspire_file_size);
			if (ret) {
		...
		case 5:
			...
			printf("Saving to %s...\n", argv[3]);
			fwrite(nspire_file_buffer, file_info->size, 1, nspire_file);
			fclose(nspire_file);
			printf("Saved!\n");
